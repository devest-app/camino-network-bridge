...
contract DvBridge is SignatureHandler, TransferManager, Context {
    ...
    function vote(uint256 vote_type, address value, bytes[] memory signatures) public payable onlyValidator(_msgSender()) returns (bool) {
        // Validation checks
        require(vote_type == 1 || vote_type == 2, "Invalid vote");

        bytes32 message = getVoteMessage(vote_type, value);
